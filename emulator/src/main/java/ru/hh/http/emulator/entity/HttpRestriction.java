...
public abstract class HttpRestriction {
	...
	public HttpRestriction(RestrictionType restrictionType,
			String key, String value, AttributeType attribyteType) {
		this.id = id;
	...
	public boolean match(final Collection<HttpEntry> request){
		...
		for (HttpEntry httpEntry : request) {
			if(httpEntry.getType().equals(attribyteType) 
					&& (httpEntry.getKey() == key || (key != null && key.equals(httpEntry.getKey())))){
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ ((attribyteType == null) ? 0 : attribyteType.hashCode());
